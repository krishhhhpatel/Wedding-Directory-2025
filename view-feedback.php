...
{
  $did = $_GET['did'];
  $q = mysqli_query($con,"delete from tbl_feedback where feedback_id = '{$did}'");
}
?>
<!--
**********************************************************************************************************
  ...
  <section id="page-banner" class="page-banner" style="background-image: url('images/banner.jpg');">
    ...
    <div class="container">
      <div class="banner-dtl text-center">
        <h2 class="banner-heading">View Feedback</h2>
      ...
      <div class="manage-item-listing-block">
        <table>
          ...
          <th class="title">Date</th>
            ...
            <th class="address">Action</th>

          </tr>
          <?php
          $uid = $_SESSION['uid'];
          $i=0;
          $q = mysqli_query($con, "select * from tbl_feedback where user_id = '{$uid}'");
          while ($data = mysqli_fetch_array($q)) {
            $i++;
            echo " <tr>
           
            <th class=' title' >{$i}</th>
            <th class=' title' >{$data['feedback_date']}</th>
            <td class=' address' >{$data['feedback_details']}</td>
            <td class=' address' ><a href='?did={$data['feedback_id']}'>Delete</a></td>
